...
export default function StatePaymentComparison() {
  ...
  const handleServiceCategoryChange = async (index: number, category: string) => {
    try {
      ...
      // If 'All States' is selected, get all codes for the category
      if (
      ...
      } else if (newFilters[index].states.length > 0) {
        // If a specific state is selected, get codes for that state and category
        const selectedState = newFilters[index].states[0];
        if (filterOptionsData) {
          const serviceCodes = Array.from(
            new Set(
              filterOptionsData.combinations
                .filter(combo => 
                  combo.service_category === category &&
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onChange={(option) => {
                            ...
                            // If user selects service code, clear service description
                            if (newValue && filterSet.serviceDescription) {
                              const newFilters = [...filterSets];
                              newFilters[index] = {
                                ...newFilters[index],
                                serviceDescription: ""
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onChange={(option) => {
                          ...
                          if (newValue && filterSet.serviceCode) {
                            const newFilters = [...filterSets];
                            newFilters[index] = {
                              ...newFilters[index],
                              serviceCode: ""
